...
const AllUsers = () => {
  ...
  const changeRole = (id, role) => {
    const token = localStorage.getItem("token");
    axios
      .patch(`${API_URL}/admin/users/${id}/role`, { role }, { headers: { Authorization: `Bearer ${token}` } })
      .then(() => {
  ...
  if (loading) return <div className="text-center py-6 text-gray-500 font-medium">Loading...</div>;
  if (error) return <div className="text-center py-6 text-red-500 font-medium">{error}</div>;

  return (
    <div className="p-4 sm:p-6 lg:p-8" data-aos="fade-up">
      ...
      <div className="overflow-x-auto rounded-lg shadow-lg border border-gray-200" data-aos="fade-up">
        <table className="min-w-full bg-white text-gray-800">
          ...
          <tbody>
            {users.length > 0 ? (
              users.map((u, idx) => (
                <tr key={u._id || idx} className={`border-b hover:bg-gray-50 ${idx % 2 === 0 ? "bg-gray-50" : "bg-white"}`}>
